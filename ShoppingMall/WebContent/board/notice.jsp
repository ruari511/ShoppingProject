			...
			<form id="sForm" name="sForm">
							...
							<c:forEach var="noticeList" items="${noticeList}">
								<tbody>
									...
									<tr class="notice_fix">
										...
										<!-- 글번호  -->
										<td class="subject"><c:if
												test="${noticeList.category eq '10' }">
												<strong class="FG01">일반</strong>
											</c:if> <c:if test="${noticeList.category eq '20' }">
												<strong class="FG02">매장</strong>
											</c:if> <c:if test="${noticeList.category eq '30' }">
												<strong class="FG03">이벤트</strong>
											</c:if> <c:if test="${noticeList.category eq '40' }">
												<strong class="FG04">뷰티테스터</strong>
											</c:if> <a href="./noticeDetail.bd?num=${noticeList.num }">${noticeList.subject }</a>
										</td>
										<!-- 글제목  -->
										<td><fmt:formatDate pattern="yyyy-MM-dd"
												value="${noticeList.reg_date }" /></td>
										<!-- 작성일  -->
									</tr>
								</tbody>
							</c:forEach>
				...
				<div class="pageing">
					...
					<a href="./notice.bd?pageNum=${firstPage }"> [첫페이지] </a>
						...
						<c:if test="${startPage != 1 }">
							<a class="prev" href="./notice.bd?pageNum=${startPage-pageBlock }"> 이전 페이지 </a>
						</c:if>
						...
						<c:if test="${endPage < finalPage}">
							<a class="next" href="./notice.bd?pageNum=${startPage+pageBlock}"> 이전 페이지 </a>
						</c:if>
				...
				<div align="center" style="padding-top: 10px;">
					...
					if(id != null){
						if(id.equals("admin")){
					%>
					<a href="./noticeWrite.bd"> 관리자 글쓰기 </a>
					<%	}
					}%>
				</div>
			</div>
