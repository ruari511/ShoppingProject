					...
					<fieldset class="search-period">
						...
						<ul class="select-month">
							<li><button type="button"
									onclick="location.href ='mypage.mp?data_month=-1'">1개월</button></li>
							<li><button type="button"
									onclick="location.href ='mypage.mp?data_month=-3'">3개월</button></li>
							<li><button type="button"
									onclick="location.href ='mypage.mp?data_month=-6'">6개월</button></li>
							<li><button type="button"
									onclick="location.href ='mypage.mp?data_month=-12'">12개월</button></li>
						</ul>
						...
						<tbody class="history">
							...
							<c:if test="${!empty buylist}">
								<c:forEach var="buylist" items="${buylist}">
									<tr>
										<td class="gubun"><span
											style="display: inline-block; font-weight: bold;"> <%-- <fmt:formatDate pattern="yyyy-MM-dd" value="${buylist.buydate }" /> --%>
												${buylist.buydate }
										</span>
										<!-- 주문일  --> <span class="color1s">${buylist.buynum }</span>
										<!-- 주문번호  --> <a class="btnDetail"
											href="mypage_BuyListDetail.mp?buynum=${buylist.buynum }">주문상세보기</a>
										</td>
										<td class="subject">
											<div class="area">
												<a class="thum"
													...
													<!-- 제품상세페이지 이동  --> <img
													src="./asset/image/${buylist.img_main }"> <!-- 제품이미지 정보 -->
												</a>
												<div class="textus">
													<a class=""
														href="ProductDetailAction.do?product_num=${buylist.product_num }">
														<span class="tit">${buylist.product_name }</span> <!-- 제품 제목  -->
														<%-- <span class="txt">${product.product_sub_name }</span> --%>
														<!-- 제품 부제목  -->
													</a>
												</div>
											</div>
										</td>
										<td class="">${buylist.buy_count }</td>
										<!-- 제품 구매 수량 -->
										<td class="colorOrange">${(buylist.product_price - buylist.product_sale_price) * buylist.buy_count }원</td>
										<!-- 총 구매가  -->
										<td><strong>${buylist.delivery_result }</strong> <!-- c:if test="{buylist.delivery_result == 1" 배송완료 -->
