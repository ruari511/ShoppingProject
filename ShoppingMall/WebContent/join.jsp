		...
		$.ajax(
			{
				...
				type:"post",
				async:false,
				data:{id:_id, pass:_pass, name:_name, email:_email, phone: _phone},
				dataType:"text",
				success:function(data,textStatus){
					...
					// 핸드폰번호 체크
					if(_phone != ""){
						if(data == 'not_overlapped'){ // 핸드폰번호가 중복이 아닐 시(사용가능)
							$("#phoneMessage").text("");
							$(".submit").attr("disabled",false);
						}else{// 중복일 때
...
<section>
		<article>
			...
			<form action="./MemberJoinAction.me" id="join" name="form" method="post">
				...
				<label>이메일</label>
					<input type="email" name="email" id="email" onblur="join_check()" required><br>
				...
				<label>휴대전화 번호 *</label>
					<select name = "phone1">
		         	...
		         	<input type = "text" name = "phone2" maxlength="4" size = "5" required/> - 
		          	<input type = "text" name = "phone3" maxlength="4" size = "5" onblur="join_check()" required/><br>
